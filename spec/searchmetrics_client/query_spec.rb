...
describe SearchmetricsClient::Query do
  let(:query) { described_class.from_hash(endpoint: 'ResearchLinksGetListExampleKey') }
  ...
  describe '#from_hash' do
    it 'requires :endpoint key' do
      expect { described_class.from_hash(params: { param1: 'val2' }) }.to raise_error(KeyError)
    end

    it 'builds query with params' do
      expect(query_with_params.query).to(
        eql('ResearchLinksGetListExampleKey.json?param1=value1&param2=a,b,c,d,e')
      )
    end
  end
